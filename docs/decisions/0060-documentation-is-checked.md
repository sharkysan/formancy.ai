...
- **Verified by:** `apps/docs/src/soup.test.ts` (6 cases: every published
  ...
  list from the server's workspace dependency closure), the decision-record and
  field-type count cases in `apps/site/src/site.test.tsx`, and
  [`CLAUDE.md`](../../CLAUDE.md), which states the obligation in the place the
  obligation is read.
...
The README keeps its literal, because a static Markdown file has no build step
to derive anything in. That is the case the check-it shape exists for, and it is
why both shapes are listed above rather than one.
